...
is inbuilt mechanism to finance cost by adding more money to circulation, it
...
high electricity consumption or in form of excessive storage media material
consumption.

Blockchain based payment systems also try to keep anonymization by using
account or wallet numbers instead of actual owner names. Therefore systems
have been used a lot for the shadow transactions not tolerating full day
light.

It might be nice to have nameless system for the device having credit used to
purchase goods and services, but does it bring any significant benefits is
open question.

It's hard to say do we really get any official legally supported electric
money which removes possibility to have full payment chain visibility or
possibility to do full payment chain resolution afterwards.
...
Island on earth last resort where we get clan chieftain mo'ai repetition when
...
Operations these several legal entities can do are, for example: exchange
(sell \& buy), tax, toll, loan, mortgage, refinance (national and central
banks), rate (national and central banks), etc. Exchange of goods and services
using money and money kind abstract valuables is based to peoples trust.
Governments have based legal framework to maintain peoples trust to these
valuables to make goods and services exchange easy and effective.

\section{Exchange}\label{exchange}\index{exchange}
...
\section{Lately development}
\label{lately_development}\index{lately development}
There is seen development where several inflation driving activities are
driven globally, while sametime reserve current interest rate is wanted to
be separated from inflation inhibitor role, led by federal bank (FED) federal
reserve chair, which makes all cash reserves and financiers having funded
lends to loose purchasing power\allowbreak\cite{FedFOMC260617}. It looks that
leader from US, which currency dollar has been reserve currency, is counting
that amount of currency carried overseas is so big, that it is beneficial to
them to initialize high inflation while maintain low federal bank overnight
interest rate, causing bigger losses overseas than in country, where
purchasing power limit is measured as montly creadit bill growth potential
comparing to existing income. Not as overseas, where it is more often measured
as real cash on account. This kind of FED habbit eats trust between the market
participants. It may be wise to plan inhibits and/or independency from this
kind of operation affects. Most likely stock prices are going up, at least
shortly, not because companies are so good, but because money is loosing value
faster than companies loose expections from they business sight in these
conditions. So, money -- basically numbers, are growing, even overall common
welth is going down for while now.
